    ...
    try:
        ...
        query = f"""
            WITH b_stats AS (
                SELECT 
                    ...
                    COALESCE(SUM(amount) FILTER (WHERE payment_status IN ('captured', 'completed')), 0) as earnings
                FROM bookings 
                WHERE {role_col} = %s
            ),
            v_stats AS (
        ...
        if isinstance(e, DatabaseConnectionError):
            ...
            try:
                ...
                earnings = sum(float(b["amount"] or 0) for b in b_data if b["payment_status"] in ["captured", "completed"])
