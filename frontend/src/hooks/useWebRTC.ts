/**
 * Web-only: 1-to-1 WebRTC video call with Supabase Realtime signaling.
 * Uses peer-to-peer connection; no third-party video SDK.
 */
...
function useWebRTC({ roomId, userId, onEnd }: UseWebRTCOptions): UseWebRTCReturn {
  ...
  const startCall = useCallback(async () => {
    ...
    if (!isSupabaseConfigured() || !getSupabase()) {
      setErr('config', 'Video calling is not configured. Add Supabase URL and anon key to .env');
      return;
    }
